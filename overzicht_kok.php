...
//    UPDATE BESTELLING WANNEER GESERVEERD IS
    $db->where ('bestellingscode', $bestellingscode);
    $db->update ('bestelling', $data);
}

//OPHALEN VAN DE BESTELLINGEN
$bestellingen = $db->objectBuilder()->rawQuery("SELECT bestelling.*, menuitem.menuitem, menuitem.gerechtcode FROM bestelling INNER JOIN menuitem ON bestelling.menuitemcode = menuitem.menuitemcode WHERE klaar = 0 AND DATE(bestelling.datum_tijd) = CURDATE() AND MINUTE(TimeDiff( NOW(), bestelling.datum_tijd )) < 990 ORDER BY bestelling.datum_tijd ASC");
